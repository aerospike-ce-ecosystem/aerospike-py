...
def extract_all_keys(request: dict) -> dict[str, list[str]]:
    ...
    for ad in candidates:
        ...
        result["set_1"].append(ad_id)
        result["set_2"].append(adgroup_id)
        result["set_3"].append(campaign_id)
        result["set_4"].append(f"{ad_id}_{channel_id}")
        result["set_5"].append(f"{adgroup_id}_{channel_id}")
        result["set_6"].append(f"{campaign_id}_{channel_id}")
        result["set_7"].append(nv_mid)
        result["set_9"].append(nv_mid)
        result["set_8"].append(str(bcookie))
