...
2. **Lazy dict conversion.** `batch_read()` returns a `LazyBatchRecords` handle (Arc-wrapped, ~10 μs). The Python-dict materialization happens lazily when the caller invokes `.to_dict()`; `.to_numpy(dtype)` provides the GIL-released structured-array path. The official client builds the full dict on I/O completion.
