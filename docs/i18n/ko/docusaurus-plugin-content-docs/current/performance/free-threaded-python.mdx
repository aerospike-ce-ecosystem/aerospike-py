...
2. **Lazy dict 변환.** `batch_read()`는 `LazyBatchRecords` (Arc-wrap, ~10 μs)를 반환. Python dict materialization은 호출자가 `.to_dict()`를 호출할 때 lazy하게 수행 — eager 작업 회피. 공식 client는 I/O 완료 즉시 전체 dict를 빌드.
