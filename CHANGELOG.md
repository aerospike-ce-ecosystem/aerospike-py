...
### Changed
- **`Query.foreach()` / `AsyncQuery.foreach()` now stream — behaviour change for existing users.** `execute_foreach` previously called `execute_query_collect`, draining the entire record stream into a `Vec` before invoking the callback once. `foreach` is the API users reach for precisely to *avoid* materialising a large scan, and it provided none of that: peak memory was the whole result set regardless of what the callback did, time-to-first-callback was the full scan duration, and the documented `False` early exit broke out of an in-memory loop after every record had already been fetched and converted. Records are now delivered as they arrive. Measured on a 60 000-record scan against a live server, fresh process each time: peak RSS growth **65.3 MB → 2.2 MB**, whole-scan wall time **141 ms → 110 ms** (conversion overlaps the network instead of following it), time-to-first-callback **127.5 ms → 8.9 ms**. Returning `False` on the first record now costs 3% of a full scan instead of 30%, and drops the stream, whose `Recordset::drop` closes the recordset and stops the server-side commands. Peak memory is bounded by `record_queue_size` (`QueryPolicy`, default 1024 records) rather than by the result set.

    **A failing query may now raise after processing part of the result set.** Because the scan stays open for as long as the callback takes, a query error — a `total_timeout` expiring, a node going away, any transport fault — now raises **after** an arbitrary prefix of records has already reached the callback. Previously the fetch finished before the first callback ran, so such an error was decided before any callback: you got either zero callbacks plus an exception, or a complete iteration, never a partial one. The exception type and message are unchanged, so nothing at the call site reveals the difference. Bounded memory and all-or-nothing delivery cannot both hold. Measured against a live server over 40 000 records with `total_timeout` tuned to fire mid-scan: buffered raised with **0** callbacks; streaming raised with **7 746** at a 25% budget and **18 684** at 50%. Stopping the server mid-scan shows the same shape (buffered completed normally with 20 000 callbacks; streaming raised after 13 697).

    This is separate from a callback raising on its own, which has **always** left preceding records processed — the old code looped over an already-filled `Vec`, so that path is unchanged.

    **Migration:** callbacks that only read (print, count, collect, search-and-stop — the common case) are unaffected and simply gain the memory and latency improvements. If your callback has side effects that must not be applied to a partial result set, use `results()` instead: it is unchanged, still materialises everything before returning, and gives the previous all-or-nothing semantics.

    The callback still runs on the calling thread, outside the Tokio runtime, so callbacks that call back into the client keep working; that is pinned by a test.
