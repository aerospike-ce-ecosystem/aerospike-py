...
- Native Rust MSRV raised to **1.85** (`rust-version = "1.85"` in `rust/Cargo.toml`, mirrored by the `MSRV` env in the CI `msrv-check` job). The resolved dependency tree pulls in `block-buffer 0.12.0` (via `digest 0.11`), whose manifest declares `edition = "2024"`; Cargo cannot parse that manifest before 1.85 (edition 2024 was stabilised in Rust 1.85), so the previous `1.80` floor made the `msrv-check` job fail with `feature 'edition2024' is required`. This dependency constraint subsumes the older 1.80 floor the crate's own code needed (std items stabilised in 1.77 / 1.80, plus `Mutex::clear_poison()` (1.74+) used by `LazyBatchRecords::release_cache`'s poison-recovery path). Only affects `pip install --no-binary` / `cargo install` builds; the published wheels remain compatible with their existing Python version range.
